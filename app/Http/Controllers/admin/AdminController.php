...
class AdminController extends Controller
{
    ...
    public function pubStore(Request $request)
    {
        ...
        for ($i = 0; $i < $max; $i++) {
            $pub = Pub::create($data);
            return redirect()->route('admin.pub')->with('success', 'la pub: ' . $pub->name . ' a été ajouté.');
        }
        session()->flash('error', 'Nombre maximum de pubs atteint.');
        return redirect()->route('admin.pub')->with('error', 'Nombre maximum de pubs atteint.');
    }
    ...
    public function pubUpdate(Request $request, Pub $pub)
    {
        ...
        $pub->update($data);
        return redirect()->route('admin.pub')->with('success', 'la pub: ' . $pub->name . ' a été mise à jour.');
    }
    ...
    {
        $pub->delete();
        return redirect()->route('admin.pub')->with('success', 'la pub: ' . $pub->name . ' a été supprimé.');
    }
